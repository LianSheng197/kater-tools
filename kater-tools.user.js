...
// @compatible   chrome Chrome 71 + Tampermonkey + v0.3.3 可正常使用
// @compatible   firefox Firefox 70 + Tampermonkey + v0.3.3 可正常使用

// @license      MIT
// ==/UserScript==

// Todo List
// 0.4 Filter notifications.
...
(function () {
  ...
  let alt_notif_main = `
  <li id="us_altNotif" class="item-notifications"><div class="ButtonGroup Dropdown dropdown NotificationsDropdown itemCount0"><button class="Dropdown-toggle Button Button--flat" data-toggle="dropdown" title="Notifications" aria-expanded="true"><i class="icon far fa-bell Button-icon"></i> <span class="Button-label">Notifications</span></button><div class="Dropdown-menu Dropdown-menu--right"><div class="NotificationList"><div class="NotificationList-header"><div class="App-primaryControl"><button class="Button Button--icon Button--link hasIcon" title="Mark All as Read" type="button" aria-label="Mark All as Read"><i class="icon fas fa-check Button-icon"></i></button></div><h4 class="App-titleControl App-titleControl--text">Notifications</h4></div><div class="NotificationList-content"></div></div></div></div></li>
  `;

  if (dev) {
    setInterval(function () {
      let origin_notif = document.querySelectorAll("li.item-notifications:not(#us_altNotif)");
      let url = location.href;
      if (origin_notif.length != 0) {
        origin_notif[0].style.display = "none";
        // 強制固定位置（第 2 個）
        if (document.querySelector("ul.Header-controls > :nth-child(2)").id != "us_altNotif") {
          if (document.querySelectorAll("li#us_altNotif").length != 0) {
            document.querySelector("li#us_altNotif").remove();
          }
          addHTML(alt_notif_main, "li.item-notifications", "beforebegin");
        }

        if (document.querySelectorAll("li#us_altNotif.item-notifications").length == 0) {
          // 檢查網址是否變動以觸發通知更新
          setInterval(function () {
